...
| M4 Airlock & Gateway | 🟡 In progress (T1–T5 done; T6–T8 pending) — T1 (`5f829b7` + Gate G1 rule 5 prep `54d8d7c`) shipped the §21.3 path containment library at `internal/airlock/paths`: `Resolve` (path arithmetic), `Validate` (mode + structural), `OpenNoFollow` (`O_NOFOLLOW` via build-tag-gated wrappers — the only `internal/` files that may import `syscall`). Property test (`testing/quick`, 200 iters) over the adversarial path corpus; round-trip test; behavioral `O_NOFOLLOW` test (skips on unsupported GOOSes); setuid/setgid rows self-skip on hosts whose filesystem drops the bit on `chmod`. T2 (`0f727b0`) shipped the ingress pipeline: `fsnotify` watcher on `<state>/workspace/inbox/`, per-pipeline scanner registry (size + zipbomb + heuristic), `quarantined_files` table (migration 0008), adversarial-corpus test. T3 (`28e53cc`) shipped the pluggable scanners: in-tree `size` / `zipbomb` / heuristic, `cmd/athanor/scanners/` for ClamAV + YARA adapters (Gate G1 allowlist extension for the `os/exec` directory), scanner-absent degrades to `VerdictUncertain` at scan time (fail-closed). T4 (`3b41b89`) shipped the egress pipeline: event-driven exporter on `category=artifact, event=accepted`, manual `athanor export` CLI subcommand, atomic temp+rename, SHA-12 idempotency. T5 (`ab082e8` + `225ed38`) shipped the §21.5 Internet Gated Reader per [ADR-0017](docs/adr/0017-gateway.md): `internal/gateway/` package owns the pure `Policy` (suffix-match allowlist with explicit `.` separator, deny-list override, non-routable CIDR guard, IDN fails closed), per-host token-bucket rate limiter, fixed deny-list header sanitizer, streamed response size cap (truncate + flag, do not fail), `network` event log. ~30 `Test*` functions in `internal/gateway`; daemon wires the gateway at boot (`cmd/athanor/gateway.go`). Gate G1 re-proven at every T5 commit. T6 (Reader Mode extraction, `go-readability` + `bluemonday`), T7 (gateway-backed tools `fetch_url` / `search_web`), T8 (adversarial security suite) remain. Gate G4 closes after the adversarial suite is green and a research goal completes end-to-end using only allowlisted domains. |
